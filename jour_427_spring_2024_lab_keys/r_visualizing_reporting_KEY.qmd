...
#Olivia
#For coloring the bars, I'd use fill= instead of weight=
arrests %>%
...
#Error in `fortify()`: ! `data` must be a <data.frame>, or an object coercible by `fortify()`, not a <table> object.  
...
#
# The chart is wrong because you didn't use slice_max to build the table. You used head and it came up with the wrong sorting. See the correct code.
all_dispo <- SF %>% 
select(disposition) %>% 
...
#Olivia create same barchart but with descending bars
all_dispo %>% 
